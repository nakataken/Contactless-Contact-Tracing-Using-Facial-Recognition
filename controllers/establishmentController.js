  ...
  Establishment.findOne({ email: email }, async (err, data) => {
    if (data) {
      ...
      if (auth) {
        ...
        res.cookie("jwtEstablishment", token, {
          httpOnly: true,
          maxAge: maxAge * 1000,
        });
        res.redirect("/establishment/dashboard");
...
const qr_post = (req, res) => {
  try {
    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, decodedToken) => {
      ...
      if (visitor_id.match(/^[0-9a-fA-F]{24}$/))
        visitor = await Visitor.findById(visitor_id);
      ...
      if (visitor) {
        if (establishment.limitVaccinated) {
          if (visitor.isVaccinated) {
            ...
            res.json({
              success: true,
              message: `${visitor.name.fname} ${visitor.name.lname}, you are not vaccinated.`,
            });
          } else {
            res.json({
              success: false,
              message: `${visitor.name.fname} ${visitor.name.lname}`,
            });
          }
        } else {
          const record = new Record({ visitor_id, establishment_id });
